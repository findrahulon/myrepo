"""RAGnarok API Gateway â€” routes requests to downstream microservices."""
...
async def query(req: QueryRequest, token_payload: dict = Depends(validate_token)):
    """Full RAG pipeline: retrieve â†’ LLM â†’ cite â†’ explain â†’ score â†’ audit."""
...
async def upload_document(
    ...
    """Upload a document â†’ store in MinIO â†’ chunk â†’ embed."""
    user = extract_user_info(token_payload)
    if user["access_level"] != "admin":
...
async def ingest_url(
    ...
    """Ingest a webpage URL â†’ clean text â†’ store in MinIO â†’ chunk â†’ embed."""
    user = extract_user_info(token_payload)
    if user["access_level"] != "admin":
